...
onpos(Board,[Rank, File],X) :- nth(Board,Rank,Squares), !, nth(Squares, File, X), !.
rank(0). rank(1). rank(2). rank(3). rank(4). rank(5). rank(_) :- !, fail.
...
boardsequalat(Where,One,Another) :- onpos(One,Where,X), !, onpos(Another, Where, X).
boardsequal(One,Another) :- board(One), board(Another), One = Another, !, forall(pos(X),boardsequalat(X,One,Another)).
